...
class User  {	
	...
	public function resetPassword(){
		...
		if($_POST['email'] == '') {
			...
			if($numRows) {			
				...
				if ($user > 0) {
					header("location:reset_password.php?authtoken=".$user['email']."");
				}			
			} else {
	...
	public function addUser () {
		if($_POST["email"]) {
			// $authtoken = $this->getAuthtoken($_POST['email']);
			$insertQuery = "INSERT INTO ".$this->userTable."(first_name, last_name, email, gender, password, mobile, designation, type, status) 
				VALUES ('".$_POST["firstname"]."', '".$_POST["lastname"]."', '".$_POST["email"]."', '".$_POST["gender"]."', '".md5($_POST["password"])."', '".$_POST["mobile"]."', '".$_POST["designation"]."', '".$_POST['user_type']."', 'active')";
			$userSaved = mysqli_query($this->dbConnect, $insertQuery);
		}
	}
